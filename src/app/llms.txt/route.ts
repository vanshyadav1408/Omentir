...
Connected agents can update Workspace, analyze a website into the product profile, create classic lead finders, Steal Customers, or outreach-only CSV agents, attach custom sequences, list and export leads, send or stop queued outreach, read the live LinkedIn inbox, reply in existing threads (with user approval), and switch the same token to another workspace the owner already has. They cannot create Omentir accounts, change billing, connect LinkedIn, create or delete workspaces, or mint API keys. Use MCP or REST for authenticated workspace data. Do not scrape /overview, /leads, or /messages.
